...
def compute_daily_sentiment(news_df, ticker=None, day_col="pub_day", text_col="text", score_col=None):
    """
    Compute daily sentiment for news articles.

    Priority:
    1. If `score_col` is provided and present, use its daily mean.
    2. If a Hugging Face API key is present in environment variables, call the
       Hugging Face Inference API for `ProsusAI/finbert` and average scores per day.
    3. Fallback to VADER (if installed).
    4. Fallback to zero sentiment per day.
    """
    df = news_df.copy()
    df.columns = [c.strip() for c in df.columns]
    ...
    # Try Hugging Face ProsusAI/finbert via Inference API if API key is available
    hf_key = os.environ.get('HUGGINGFACE_API_KEY') or os.environ.get('HF_API_KEY') or os.environ.get('HUGGINGFACE_TOKEN')
    if hf_key:
        # if the env var contains multiple keys comma-separated, take the first
        if ',' in hf_key:
            hf_key = hf_key.split(',')[0].strip()
        ...
        for i in range(0, len(texts), batch_size):
            ...
            try:
                ...
                if r.status_code != 200:
                    print(f"HF inference API error {r.status_code}: {r.text}", file=sys.stderr)
                    raise RuntimeError("HF inference failed")
                res = r.json()
            except Exception as e:
                print(f"Hugging Face inference failed: {e}", file=sys.stderr)
                res = None
...
def main():
    ...
    # outdir can be provided, otherwise will be derived from ticker after parsing
    ap.add_argument("--outdir", default=None, help='Output directory (defaults to output/<TICKER>/lstm_finbert)')
    args = ap.parse_args()
    if args.outdir is None:
        args.outdir = os.path.join("output", args.ticker, "lstm_finbert")
