...
def build_lstm(input_len, n_features, lr=1e-3):
...
def main():
    ...
    args = ap.parse_args()
    ap.add_argument("--outdir", default=os.path.join("output", args.ticker,"lstm_advanced"))
    args = ap.parse_args()
    ...
    model = build_lstm(args.seq_len, n_features=feats.shape[1], lr=1e-3)
    ...
    try:
        ...
        pass
    print(f"[LSTM Advanced] MAE={mae:.6f}  MAPE={mape:.6f}  Acc≈{acc:.6f}")
